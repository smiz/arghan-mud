...
Room(graph,room_description(),MAZE_START_ROOM_NUMBER) {
    ...
    for (int j = 0; j < DEPTH; j++) {
        ...
        num_rooms = 0;
        z++;
        Room* room = new Room(graph,room_description(),room_number++);
        maze[x][y][z] = room;
        direction.dir = Up;
        direction.id = origin->get_group()->group_number();
        direction.description = "A set of stairs winds up.";
        room->get_group()->add_direction(direction);
        direction.dir = Down;
        direction.id = room->get_group()->group_number();
        direction.description = "A set of stairs descends deeper into the maze.";
        origin->get_group()->add_direction(direction);
        origin = room;
    }
}